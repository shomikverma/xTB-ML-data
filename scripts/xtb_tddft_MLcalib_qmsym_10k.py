...
def analyze_preds():
    ...
    df = pd.read_csv('QMsym_10k_all_preds.csv')
    S1xtbs = df['xtb_S1']
    S1xtb_Lins = df['lin_S1_preds']
    S1xtb_MLs = df['C2_S1_preds']
    S1TDDFTs = df['TDDFT_S1']
    ...
    plt.savefig('plots/SCOP_QM_ML_comp.pdf')
...
def analyze_preds_old():
    ...
    with open('QMsym_10k_xtb_calib.csv', 'r') as file:
        data = file.readlines()
        for line in data:
    ...
    with open('QMsym_10k_sep_xtb_calib.txt', 'w') as file:
    ...
    plt.plot(T1xtbs, T1TDDFTs, 'r.', markersize=2, label='orig')
    plt.plot(T1xtb_Lins, T1TDDFTs, 'g.', markersize=2, label='lin calib')
    plt.plot(T1xtb_MLs, T1TDDFTs, 'b.', markersize=2, label='ML calib')
    x = np.linspace(0, 10, 100)
    plt.plot(x, x, 'k--')
    plt.xlim(0, 10)
    plt.ylim(0, 10)
    ...
    fig = plt.figure(num=1, clear=True)
    ax = fig.add_subplot(1, 1, 1)
    plt.plot(S1xtbs, S1TDDFTs, 'r.', markersize=2, label='orig')
    plt.plot(S1xtb_Lins, S1TDDFTs, 'g.', markersize=2, label='lin calib')
    plt.plot(S1xtb_MLs, S1TDDFTs, 'b.', markersize=2, label='ML calib')
    x = np.linspace(0, 10, 100)
    plt.plot(x, x, 'k--')
    plt.xlim(0, 10)
    plt.ylim(0, 10)
    ...
    with open('QMsym_10k_xtb_calib_multi.csv', 'r') as file:
        data = file.readlines()
        for line in data:
            if 'SMILES' in line or 'smiles' in line:
                continue
    ...
    with open('QMsym_10k_xtb_calib_multi.txt', 'w') as file:
        file.write('T1 orig r2:\t' + str(T1_origr2) + '\n')
        file.write('T1 lin fixed r2:\t' + str(T1_lin_fixedr2) + '\n')
        file.write('T1 fixed r2:\t' + str(T1_fixedr2) + '\n')
        file.write('T1 orig MAE:\t' + str(T1_origMAE) + '\n')
    ...
    label_axes(fig, ha='left')
    plt.savefig('plots/QMsym10k_ML_comp.png')
    ...
    # 'SMILES,CID,xtb_T1,xtb_S1,TDDFT_T1,TDDFT_S1,T1err,S1err,xtb_S1_fL,TDDFT_S1_fL,S1fLerr,'
    # 'xtb_ML_T1,xtb_ML_S1\n'
    # with open('QMsym_10k_xtb_calib_multi_hyperopt.csv', 'r') as file:
    #     data = file.readlines()
    #     for line in data:
    #         if 'SMILES' in line or 'smiles' in line:
    #             continue
    ...
    # print('S1 orig RMSE:\t' + str(S1_origRMSE))
    # print('S1 fixed RMSE:\t' + str(S1_fixedRMSE))
    # with open('QMsym_10k_multi_xtb_calib.txt', 'w') as file:
    #     file.write('T1 orig r2:\t' + str(T1_origr2) + '\n')
    #     file.write('T1 fixed r2:\t' + str(T1_fixedr2) + '\n')
    #     file.write('T1 orig MAE:\t' + str(T1_origMAE) + '\n')
    #     file.write('T1 fixed MAE:\t' + str(T1_fixedMAE) + '\n')
